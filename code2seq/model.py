...
import reader
from common import Common, Plots, GPUSelector
...
run_name = strftime("code2seq_%d-%m-%Y_%H-%M-%S")
summary_writer = tf.summary.FileWriter(f"./tensorboard_logs/{run_name}")
...
class Model:
    ...
    def train(self):
        ...
        for iteration in range(1, (self.config.NUM_EPOCHS // self.config.SAVE_EVERY_EPOCHS) + 1):
            ...
            try:
                ...
                accuracy, precision, recall, f1, y_true, y_pred, y_prob = self.evaluate()
                self.log_results_to_wandb_and_tensorboard(accuracy, batch_loss, f1, precision, recall, y_prob, y_pred, y_true)

                if self.config.BEAM_WIDTH == 0:
                    print('Accuracy after %d epochs: %.5f' % (self.epochs_trained, accuracy))
                else:
                    print('Accuracy after {} epochs: {}'.format(self.epochs_trained, accuracy))
    ...
    def log_results_to_wandb_and_tensorboard(self, accuracy: float, batch_loss: float, f1: float, precision: float, recall: float, y_prob: list, y_pred: list, y_true: list):
        ...
        if not y_true:
            ...
            y_pred = ["nobug"]
            y_prob = [1.0]
        print("Creating plots for epoch: %d" % self.epochs_trained)
        plots = Plots(self.config)
        plots.plot_everything(y_prob=y_prob, y_pred=y_pred, y_true=y_true)
        # Record to WanDB
        wandb.log({'loss': batch_loss, 'val_f1': f1,
        ...
        summary_writer.add_summary(summary, self.epochs_trained)
        buf = io.BytesIO()
        ...
        # Add image summary
        summary_op = tf.summary.image("plots", image, family="plots")
        with tf.Session() as sess:
            summary_writer.add_summary(summary_op.eval(), self.epochs_trained)
        # Flush Tensorboard
    ...
    def evaluate(self, release=False):
        ...
        if self.eval_queue is None:
            ...
            self.eval_predicted_indices_op, self.eval_topk_values, _, _, self.eval_probs = \
        ...
        if self.config.LOAD_PATH and not self.config.TRAIN_PATH:
            ...
            if release:
                ...
                return None
        if self.config.SAVE_PATH:
            model_dirname = os.path.dirname(self.config.SAVE_PATH)
        elif self.config.LOAD_PATH:
            model_dirname = os.path.dirname(self.config.LOAD_PATH)
        else:
            model_dirname = os.path.dirname(os.getcwd())
        ...
        with open(model_dirname + '/log.txt', 'w') as output_file, open(ref_file_name, 'w') as ref_file, open(
            ...
            # Holds all gold/true labels from evaluation set
            y_true = []
            # Holds all predicted labels
            y_pred = []
            # For each prediction, the probability that the label is buggy
            y_prob = []

            try:
                while True:
                    predicted_indices, true_target_strings, top_values, probs = self.sess.run(
                        [self.eval_predicted_indices_op, self.eval_true_target_strings_op, self.eval_topk_values, self.eval_probs],
                    )
                    ...
                    num_correct_predictions = self.update_correct_predictions(num_correct_predictions, output_file,
                                                                              zip(true_target_strings,
                                                                                  predicted_strings))
                    y_true, y_pred, y_prob = self.update_per_subtoken_statistics(
                        zip(true_target_strings, predicted_strings, probs),
        ...
        precision, recall, f1 = self.calculate_results(y_true, y_pred)
        print("Evaluation time: %sh%sm%ss" % ((elapsed // 60 // 60), (elapsed // 60) % 60, elapsed % 60))
        return num_correct_predictions / total_predictions, \
               precision, recall, f1, y_true, y_pred, y_prob
    ...
    def update_per_subtoken_statistics(self, results, y_true, y_pred, y_prob):
        bug_index = self.target_to_index[self.config.BUG_STRING]
        for original_name, predicted, prob in results:
            if self.config.BEAM_WIDTH > 0:
            ...
            predicted_name = filtered_predicted_names[0]
            y_true.append(original_name)
            y_pred.append(predicted_name)
            y_prob.append(prob[0][bug_index])
    ...
    def print_hyperparams(self):
        ...
        print('Test/Validation path:\t\t\t', self.config.TEST_PATH)
    ...
    def calculate_results(self, y_true, y_pred):
        precision = precision_score(y_true, y_pred, average='binary', pos_label=self.config.BUG_STRING)
        recall = recall_score(y_true, y_pred, average='binary', pos_label=self.config.BUG_STRING)
        f1 = f1_score(y_true, y_pred, average='binary', pos_label=self.config.BUG_STRING)
    ...
    def build_training_graph(self, input_tensors):
        ...
        with tf.variable_scope('model'):
            ...
            weights = self.calculate_weights(target_index)
            crossent = tf.losses.sparse_softmax_cross_entropy(labels=target_index, logits=logits, weights=weights)
    ...
    def calculate_weights(self, target_index):
        # Set weight for all classes to 1.0
        class_weights = [1.0] * len(self.target_to_index)

        # Find out indexes to bugs and non-bugs
        bug_label_index = self.target_to_index[self.config.BUG_STRING]
        """ Uncomment if you want to set weights proportionally
        For example, ratio 1:10 (bug:nobug), then bug will get weight 10/1 = 10.0 and nobug will have weight 1.0
        
        nobug_label_index = self.target_to_index[self.config.NO_BUG_STRING]

        # Find out the number of buggy and non-buggy examples
        bin_count = tf.math.bincount(target_index)
        nr_bug_labels = bin_count[bug_label_index]
        nr_nobug_labels = bin_count[nobug_label_index]
        # Avoid division by zero later, sometimes in small batches there are no bug labels
        tf.cond(tf.math.equal(nr_bug_labels, 0), lambda: tf.add(nr_bug_labels, 1), lambda: tf.add(nr_bug_labels, 0))
        # Set weights and for bugs, which should be minority, give higher weight/coefficient for cross entropy
        bug_weight = tf.math.divide(nr_nobug_labels, nr_bug_labels)
        tf.dtypes.cast(bug_weight, tf.float16)
        class_weights[bug_label_index] = bug_weight
        """
        class_weights[bug_label_index] = self.config.BUG_WEIGHT_FOR_LOSS
        weights = tf.gather(class_weights, target_index)
