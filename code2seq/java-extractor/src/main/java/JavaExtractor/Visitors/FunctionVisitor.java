...
public class FunctionVisitor extends VoidVisitorAdapter<Object> {
    ...
    private void visitMethod(MethodDeclaration rootNode) {
        LeavesCollectorVisitor leavesCollectorVisitor = new LeavesCollectorVisitor();
        leavesCollectorVisitor.visitPreOrder(rootNode);
        ArrayList<Node> leaves = leavesCollectorVisitor.getLeaves();

        String normalizedMethodName = Common.normalizeName(rootNode.getName().asString(), Common.BlankWord);
        ArrayList<String> splitNameParts = Common.splitToSubtokens(rootNode.getName().asString());
        ...
        if (rootNode.getBody().isPresent()) {
            long methodLength = getMethodLength(rootNode.getBody().toString());
            if (m_CommandLineValues.MaxCodeLength > 0) {
                if (methodLength >= m_CommandLineValues.MinCodeLength && methodLength <= m_CommandLineValues.MaxCodeLength) {
                    m_Methods.add(new MethodContent(rootNode, leaves, splitName));
                }
            } else {
                m_Methods.add(new MethodContent(rootNode, leaves, splitName));
            }
