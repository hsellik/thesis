...
class Config:
    ...
    def arguments_parser(cls) -> ArgumentParser:
        ...
        parser.add_argument('--preprocess', dest='preprocess', required=False,
                            help="preprocess data before starting to train the model")
        parser.add_argument('--realistic_ratio', dest='realistic_ratio', required=False,
                            help="preprocess data with realistic ratio")
        return parser
    ...
    def set_imbalanced_off_by_one_config(self):
        print("Setting imbalanced off-by-one config")
        self.NUM_TRAIN_EPOCHS = 54
        self.SAVE_EVERY_EPOCHS = 1
        self.TRAIN_BATCH_SIZE = 1024
        self.TEST_BATCH_SIZE = self.TRAIN_BATCH_SIZE
        self.TOP_K_WORDS_CONSIDERED_DURING_PREDICTION = 1
        ...
        # model hyper-params
        self.MAX_CONTEXTS = 200
        self.MAX_TOKEN_VOCAB_SIZE = 1350000
        self.MAX_TARGET_VOCAB_SIZE = 2
        self.MAX_PATH_VOCAB_SIZE = 900000
        self.DEFAULT_EMBEDDINGS_SIZE = 104
        self.TOKEN_EMBEDDINGS_SIZE = self.DEFAULT_EMBEDDINGS_SIZE
        self.PATH_EMBEDDINGS_SIZE = self.DEFAULT_EMBEDDINGS_SIZE
        self.CODE_VECTOR_SIZE = self.context_vector_size
        self.TARGET_EMBEDDINGS_SIZE = self.CODE_VECTOR_SIZE
        self.DROPOUT_KEEP_RATE = 0.6046
        self.SEPARATE_OOV_AND_PAD = False
        self.PATIENCE = 3

    def set_balanced_off_by_one_config(self):
        print("Setting balanced off-by-one config")
        self.NUM_TRAIN_EPOCHS = 54
        self.SAVE_EVERY_EPOCHS = 1
        self.TRAIN_BATCH_SIZE = 1551
        self.TEST_BATCH_SIZE = self.TRAIN_BATCH_SIZE
        self.TOP_K_WORDS_CONSIDERED_DURING_PREDICTION = 1
        self.NUM_BATCHES_TO_LOG_PROGRESS = 100
        self.NUM_TRAIN_BATCHES_TO_EVALUATE = 1800
        ...
        # model hyper-params
        self.MAX_CONTEXTS = 154
        self.MAX_TOKEN_VOCAB_SIZE = 1191566
        self.MAX_TARGET_VOCAB_SIZE = 2
        self.MAX_PATH_VOCAB_SIZE = 629050
        self.DEFAULT_EMBEDDINGS_SIZE = 106
        self.TOKEN_EMBEDDINGS_SIZE = self.DEFAULT_EMBEDDINGS_SIZE
        self.PATH_EMBEDDINGS_SIZE = self.DEFAULT_EMBEDDINGS_SIZE
        self.CODE_VECTOR_SIZE = self.context_vector_size
        self.TARGET_EMBEDDINGS_SIZE = self.CODE_VECTOR_SIZE
        self.DROPOUT_KEEP_RATE = 0.6551
        self.SEPARATE_OOV_AND_PAD = False
        self.PATIENCE = 3

    def load_from_args(self, set_defaults: bool = False):
        args, unknown = self.arguments_parser().parse_known_args()
        # Automatically filled, do not edit:
        self.PREDICT = args.predict
        self.MODEL_SAVE_PATH_PREFIX = args.save_path
        self.MODEL_LOAD_PATH_PREFIX = args.load_path
        ...
        self.PREPROCESS = False if not args.preprocess else args.preprocess
        self.REALISTIC_RATIO = False if not args.realistic_ratio else args.realistic_ratio
        if set_defaults:
            self.set_defaults()
        else:
            if args.imbalanced:
                self.set_imbalanced_off_by_one_config()
            else:
                self.set_balanced_off_by_one_config()
    ...
    def __init__(self, set_defaults: bool = False, load_from_args: bool = False, verify: bool = False):
        ...
        self.PICK_BEST_GPU: bool = False
        self.BUG_STRING: str = ""
        self.NO_BUG_STRING: str = ""
        ...
        self.MODEL_SAVE_PATH_PREFIX: Optional[str] = None
        self.MODEL_LOAD_PATH_PREFIX: Optional[str] = None
        ...
        self.PREPROCESS: bool = False
        self.REALISTIC_RATIO: bool = False
        ...
        if load_from_args:
            self.load_from_args(set_defaults=set_defaults)
        if verify:
            self.verify()
    ...
    def is_loading(self) -> bool:
        return bool(self.MODEL_LOAD_PATH_PREFIX)

    @property
    def is_saving(self) -> bool:
        return bool(self.MODEL_SAVE_PATH_PREFIX)
    ...
    def model_load_dir(self):
        return '/'.join(self.MODEL_LOAD_PATH_PREFIX.split('/')[:-1])

    @property
    def entire_model_load_path(self) -> Optional[str]:
        if not self.is_loading:
            return None
        return self.get_entire_model_path(self.MODEL_LOAD_PATH_PREFIX)

    @property
    def model_weights_load_path(self) -> Optional[str]:
        if not self.is_loading:
            return None
        return self.get_model_weights_path(self.MODEL_LOAD_PATH_PREFIX)

    @property
    def entire_model_save_path(self) -> Optional[str]:
        if not self.is_saving:
            return None
        return self.get_entire_model_path(self.MODEL_SAVE_PATH_PREFIX)

    @property
    def model_weights_save_path(self) -> Optional[str]:
        if not self.is_saving:
            return None
        return self.get_model_weights_path(self.MODEL_SAVE_PATH_PREFIX)
