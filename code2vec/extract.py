...
def ExtractFeaturesForDir(args, dir, prefix):
    command = ['java', '-cp', args.jar, 'JavaExtractor.App', '--code2vec', str(args.code2vec),
               '--max_path_length', str(args.max_path_length), '--max_path_width', str(args.max_path_width),
               '--dir', dir, '--num_threads', str(args.num_threads), '--off_by_one', str(args.off_by_one),
               '--nullpointer', str(args.nullpointer), '--realistic_bug_ratio', str(args.realistic_bug_ratio)]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--code2vec", dest="code2vec", required=False, default="true")
    ...
    parser.add_argument("--off_by_one", dest="off_by_one", required=False, default="false")
    parser.add_argument("--nullpointer", dest="nullpointer", required=False, default="false")
    parser.add_argument("--realistic_bug_ratio", dest="realistic_bug_ratio", required=False, default="false")
    args = parser.parse_args()

    if args.file is not None:
        command = 'java -cp ' + args.jar + ' JavaExtractor.App --code2vec ' + str(args.code2vec) + ' --max_path_length ' + \
                  str(args.max_path_length) + ' --max_path_width ' + str(args.max_path_width) + ' --file ' + args.file  + \
                  ' --off_by_one ' + args.off_by_one + ' --nullpointer ' + args.nullpointer + \
                  ' --realistic_bug_ratio ' + args.realistic_bug_ratio
        os.system(command)
