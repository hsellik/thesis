...
class Evaluator:
    ...
    def __init__(self, config: Config, model: Code2VecModelBase):
        self.model = model
        self.config = config

    def evaluate(self):
        evaluation_results = self.model.evaluate()
        test_accuracy = evaluation_results.subtoken_accuracy
        test_precision = evaluation_results.subtoken_precision
        test_recall = evaluation_results.subtoken_recall
        test_f1 = evaluation_results.subtoken_f1
        test_y_prob = evaluation_results.y_prob
        test_y_pred = evaluation_results.y_pred
        test_y_true = evaluation_results.y_true
        self.print_statistics(test_accuracy, test_precision, test_recall, test_f1, test_y_true, test_y_pred)
        self.log_results_to_wandb(test_accuracy, test_f1, test_precision, test_recall, test_y_prob, test_y_pred,
                                  test_y_true)

    def print_statistics(self, accuracy, precision: float, recall: float, f1: float, y_true: list, y_pred: list):
        print(
            'Accuracy: ' + str(accuracy) + 'Precision: ' + str(precision) + ', Recall: ' + str(recall) + ', F1: ' + str(
                f1))
        tn, fp, fn, tp = confusion_matrix(y_true, y_pred,
                                          labels=[self.config.NO_BUG_STRING, self.config.BUG_STRING]).ravel()
    ...
    def log_results_to_wandb(self, accuracy: float, f1: float, precision: float, recall: float,
                             y_prob: list, y_pred: list, y_true: list):
