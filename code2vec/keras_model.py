...
class Code2VecModel(Code2VecModelBase):
    ...
    def _create_train_callbacks(self) -> List[Callback]:
        ...
        if self.config.EARLY_STOPPING:
            keras_callbacks.append(tf.keras.callbacks.EarlyStopping(monitor='loss', patience=self.config.PATIENCE))
...
class ModelEvaluationCallback(MultiBatchCallback):
    ...
    def perform_evaluation(self):
        ...
        wandb.log({'val_loss': evaluation_results.loss, 'val_f1': evaluation_results.subtoken_f1,
                   'val_recall': evaluation_results.subtoken_recall, 'val_precision': evaluation_results.subtoken_precision})
